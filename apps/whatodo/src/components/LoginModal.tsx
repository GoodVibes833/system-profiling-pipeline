    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-6 relative">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl font-black text-slate-900 mb-1">캐나다가자에 오신 것을 환영해요!</h2>
          <p className="text-sm text-slate-500">
            {message || "로그인하면 랭킹, 친구 기능을 모두 이용할 수 있어요"}
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 py-3.5 px-4 bg-white border-2 border-slate-200 rounded-2xl font-bold text-slate-700 hover:border-slate-300 hover:bg-slate-50 transition-all disabled:opacity-50 mb-3"
        >
        ...
        <p className="text-xs text-slate-400 text-center">
          로그인하면 방문 기록, 뱃지, 랭킹이 서버에 안전하게 저장돼요
        </p>
      </div>
    </div>
  );
